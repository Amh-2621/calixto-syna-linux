...
static struct ispbe_isr_dbg g_ispbe_isr_dbg;
...
static int ISPBE_ISR_Handler_Task(void)
{
	MV_CC_MSG_t msg;
	int rc;
	enum ispbe_isr_type isr_type;

	while (!g_ispbe_ca_ctx.isrTaskExitFlag) {
		rc = ispss_wait_dhub_sem_intr(&msg);
		if (!rc) {
			/* Determine ISR type based on interrupt number */
			switch (msg.m_Param2) {
			case ISP_DHUBSEM_TSB_tile_intr:
				isr_type = ISPBE_ISR_TILER;
				break;
			case ISP_DHUBSEM_TSB_dwrp_intr:
				isr_type = ISPBE_ISR_DEWARP;
				break;
			default:
				isr_type = ISPBE_ISR_MAX; // Invalid
				break;
			}

			if (isr_type < ISPBE_ISR_MAX) {
				ispbe_isr_dbg_start(&g_ispbe_isr_dbg, isr_type, msg.m_Param2);
			}

			ISPBE_ProcessISR(msg.m_Param2, &msg);

			if (isr_type < ISPBE_ISR_MAX) {
				ispbe_isr_dbg_end(&g_ispbe_isr_dbg, isr_type);
			}
		}
	}
	return 0;
...
INT ISPBE_CA_Initialize(void)
{
	...
	/* Initialize ISR debugfs */
	ispbe_isr_dbg_create(&g_ispbe_isr_dbg);
...
INT ISPBE_CA_DeInitialize(void)
{
	...
	ispss_enable_dhub_sem_intr(ISP_DHUBSEM_TSB_tile_intr, 0);
	kthread_stop(g_ispbe_ca_ctx.ispbe_isr_handler_task);

	ispbe_isr_dbg_remove(&g_ispbe_isr_dbg);
...
INT ISPBE_MODULE_Close(enum ISPBE_MODULES module, INT clientID)
{
	...
	if (drv_ctx->fops.module_close != NULL) {
		...
		if (Ret == ISPSS_EHARDWAREBUSY || Ret == ISPSS_EQNOTEMPTY)
			return Ret;
...
EXPORT_SYMBOL(ISPBE_MODULE_PopRequest);
