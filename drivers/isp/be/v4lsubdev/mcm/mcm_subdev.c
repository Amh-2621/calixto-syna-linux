...
static int mcm_subdev_buf_alloc(struct v4l2_subdev *sd, void *arg)
{
	...
	if (cur_pad->sensor_out_state == SENSOR_OUT_ENABLE_MCM_MMU_ENABLE ||
		...
		if (cur_pad->sensor_out_state == SENSOR_OUT_DISABLE_MCM_MMU_DISABLE) {
			...
			if (ispSS_SHM_Allocate(SHM_NONSECURE, size, 32,
				&new_buf->shm_handle, SHM_NONSECURE_NON_CONTIG) != SUCCESS) {
				dev_err(mcm_dev->dev, "Failed to allocate memory\n");
				kfree(new_buf);
				return -ENOMEM;
			}
...
int mcm_subdev_subscribe_event(struct v4l2_subdev *sd,
		struct v4l2_fh *fh,
...
static struct v4l2_subdev_core_ops mcm_subdev_core_ops = {
	.ioctl             = mcm_subdev_priv_ioctl,
	.subscribe_event   = mcm_subdev_subscribe_event,
	.unsubscribe_event = v4l2_event_subdev_unsubscribe,
};

static struct v4l2_subdev_video_ops mcm_subdev_video_ops = {
};

struct v4l2_subdev_ops mcm_subdev_subdev_ops = {
...
static struct v4l2_subdev_internal_ops mcm_subdev_internal_ops = {
...
int mcm_miscdev_mmap(struct file *file, struct vm_area_struct *vma)
{
	struct miscdevice *pmisc_dev = file->private_data;
	struct mcm_subdev_dev *mcm_dev;
	struct vvcam_vb2_buffer *pos;
