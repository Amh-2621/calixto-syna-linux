		...
		unsigned int cmd, unsigned long arg)
{
	...
	case ISP_SET_CLK:
	{
		...
		if (!copy_from_user(&clk_par, pInArg, sizeof(ISP_CLK_PARAMS))) {
			if ((clk_par.clk_id > -1) &&
					(clk_par.clk_id < ARRAY_SIZE(isp_clock_list))) {
	...
	case ISP_GET_CLK:
	{
		...
		if (!copy_from_user(&clk_par, pInArg, sizeof(ISP_CLK_PARAMS))) {
			if ((clk_par.clk_id > -1) &&
					(clk_par.clk_id < ARRAY_SIZE(isp_clock_list))) {
				clk_rate = clk_get_rate(isp_clks[clk_par.clk_id]);
				if (clk_rate < 0) {
					isp_error(dev,
					...
					clk_par.clk_rate = clk_rate;
					if (copy_to_user(pInArg, &clk_par,
						sizeof(ISP_CLK_PARAMS)) != 0) {
						isp_error(dev,
							"Failed to copy clk_rate to user\n");
