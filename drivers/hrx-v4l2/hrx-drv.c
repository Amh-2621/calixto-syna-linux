...
int hrx_isr_state_update(struct syna_hrx_v4l2_dev *hrx_dev, MV_CC_MSG_t msg)
{
	int hrx_intr = 0xff;
	u32 irq = 0;
	int ret, iter;
...
static SIMPLE_DEV_PM_OPS(syna_hrx_v4l2_pmops, syna_hrx_v4l2_suspend,
 ...
 #endif
void hrx_set_sram_power(struct syna_hrx_v4l2_dev *hrx_dev, unsigned int val)
{
...
static int hrx_pwr_config(const char *val, const struct kernel_param *param)
{
	int err;
	...
	if (hrx_pwr_val == 0) {
		HRX_LOG(HRX_DRV_ERROR, "power-off \n");
		hrx_set_sram_power(hrx_dev, 7);

		hrx_reg_write(hrx_dev, 0x103, HDMI_PHY_CONFIG);
		...
		HRX_LOG(HRX_DRV_ERROR, "power-on \n");
		reg_addr = MEMMAP_AVIO_REG_BASE + AVIO_MEMMAP_AVIO_GBL_BASE + \
			RA_avioGbl_HDMI_CLK_EN;
		reg_val = glb_reg_read(reg_addr);
		reg_val |= MSK32avioGbl_HDMI_CLK_EN_HdmiRx;
